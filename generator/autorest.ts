...
export async function generateAutorestConfig(readmePath: string, bicepReadmePath: string) {
  ...
  for (const node of markdown.iterate(readmeMarkdown.markDown)) {
    ...
    // js-yaml v5 throws "expected a document, but the input is empty" for empty or
    // comment-only yaml blocks (e.g. the autogenerated 'all-api-versions' placeholder
    // and '#exclude-file' blocks). Such blocks contain no input-file entries, so skip them.
    // Any other parse failure indicates genuinely malformed yaml and is rethrown so it
    // fails loudly instead of silently dropping input-file entries.
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let yamlData: any;
    try {
      yamlData = yaml.load(node.literal);
    } catch (err) {
      if (err instanceof Error && err.message.includes('input is empty')) {
        continue;
      }
      throw err;
    }
