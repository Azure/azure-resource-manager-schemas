...
import { getFullAutoGenList } from '../autogenlist';
...
executeSynchronous(async () => {
  ...
  // Preserve disabled provider refs while replacing all enabled provider refs
  const fullAutoGenList = getFullAutoGenList();
  await saveAutoGeneratedSchemaRefs(flatten(schemaConfigs), true, fullAutoGenList);
});