...
import { SchemaConfiguration, generateSchemas, clearAutoGeneratedSchemaRefs, saveAutoGeneratedSchemaRefs, generateSchemasWithCsharpGenerator } from '../generate';
...
import { executeSynchronous, chunker, writeJsonFile, lowerCaseContains } from '../utils';
...
executeSynchronous(async () => {
    ...
    if (combineBatchMode) {
        ...
        await saveAutoGeneratedSchemaRefs(flatten(schemaConfigs), true, allAutoGenConfigs);
    }
...
async function generateCsharpSchemas(logger: ILogger, combineBatchMode: boolean): Promise<SchemaConfiguration[]> {
  ...
  for (const providerNamespace of csharpGeneratorEnabledProviders) {
    try {
