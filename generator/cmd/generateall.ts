...
import { SchemaConfiguration, generateSchemas, clearAutoGeneratedSchemaRefs, saveAutoGeneratedSchemaRefs, generateSchemasWithCsharpGenerator, getCsharpGeneratorProviderNamespaces } from '../generate';
import { csharpGeneratorDisabledProviders, findOrGenerateAutogenEntries, getFullAutoGenList } from '../autogenlist';
import { clearAutorestTempDir } from '../autorest';
import colors from 'colors';
import { flatten } from 'lodash';
import { executeSynchronous, chunker, writeJsonFile, lowerCaseContains, lowerCaseEquals } from '../utils';
import { Package, AutoGenConfig } from '../models';
...
const argsConfig = yargs(process.argv.slice(2))
  ...
  .option('csharp-only', { type: 'boolean', desc: 'Only generate C# schemas, leave others as-is' });
...
executeSynchronous(async () => {
    ...
    const csharpOnly = args['csharp-only'] ?? false;
    if (csharpOnly) {
        basePaths = [];
    }
    ...
    const allAutoGenConfigs: AutoGenConfig[] = [];

    const summaryLogger = await getLogger(summaryPath);

    if (batchIndex === undefined || batchIndex === 0) {
      // if batching, only run this for the first batch - it should be quick
      const newConfigs = await generateCsharpSchemas(summaryLogger, combineBatchMode);
    ...
    for (const basePath of basePaths) {
        try {
            ...
            const fullAutoGenList = await findOrGenerateAutogenEntries(basePath, readme);
            allAutoGenConfigs.push(...fullAutoGenList);
            let filteredAutoGenList = fullAutoGenList
            ...
            if (!combineBatchMode) {
                // Pass the full list (including disabled) to preserve their refs
                await clearAutoGeneratedSchemaRefs(fullAutoGenList);
            }
    ...
    if (combineBatchMode) {
        ...
        // if we're only generating C# schemas, don't remove other existing schemas.
        const removeExistingRefs = !csharpOnly;
        // Preserve disabled provider refs in non-batch mode too
        await saveAutoGeneratedSchemaRefs(flatten(schemaConfigs), removeExistingRefs, allAutoGenConfigs);
    }
...
async function generateCsharpSchemas(logger: ILogger, combineBatchMode: boolean): Promise<SchemaConfiguration[]> {
  const schemaConfigs: SchemaConfiguration[] = [];
  let hasErrors = false;
  // Handle C# generator entries first — they don't require a readme file.

    const providerNamespaces = (await getCsharpGeneratorProviderNamespaces())
            .filter(x => !lowerCaseContains(csharpGeneratorDisabledProviders, x));

    for (const providerNamespace of providerNamespaces) {
    try {
      console.log(`Using C# generator for namespace: ${providerNamespace}`);

      if (!combineBatchMode) {
        const autogenEntries = getFullAutoGenList()
            .filter(x => lowerCaseEquals(x.namespace, providerNamespace));

        for (const entry of autogenEntries) {
            // Remove any existing entries for this provider namespace
            await clearAutoGeneratedSchemaRefs([{ namespace: entry.namespace, suffix: entry.suffix }]);
        }

        // Pass the full list (including disabled) to preserve their refs
        await clearAutoGeneratedSchemaRefs([{ namespace: providerNamespace }]);
      }

      const startTime = Date.now();
      const newConfigs = await generateSchemasWithCsharpGenerator(providerNamespace);
      const generationTime = Date.now() - startTime;
      console.log(`Time taken to generate ${colors.green.italic(providerNamespace)} : ${colors.magenta.bold(generationTime.toString())} ms.`);

      logOut(logger, `
        <details>
          <summary>Successfully generated types for namespace '${providerNamespace}' (C# generator).</summary>
        </details>
      `);

      schemaConfigs.push(...newConfigs);
    } catch (error) {
      console.log(colors.red(`Caught exception processing C# autogenlist entry ${providerNamespace}.`));
      console.log(colors.red(`${(error as Error)?.stack || error}`));
      process.exitCode = 1;
      logOut(logger, `
        <details>
          <summary>Failed to generate types for namespace '${providerNamespace}' (C# generator)</summary>
          \`\`\`
          ${error}
          \`\`\`
        </details>
