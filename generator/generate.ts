...
export async function clearAutogeneratedSchemaRefs(whitelist: WhitelistConfig[]) {
    const rootSchemaConfig = RootSchemaConfigs.get(ScopeType.ResourceGroup) as RootSchemaConfiguration;
    const currentRefs = await getCurrentTemplateRefs(ScopeType.ResourceGroup, rootSchemaConfig);
...
export async function saveAutogeneratedSchemaRefs(schemaConfigs: SchemaConfiguration[]) {
    const refsMap: Map<ScopeType, string[]> = new Map();
    [ScopeType.ResourceGroup, ScopeType.Subcription, ScopeType.Tenant, ScopeType.ManagementGroup]
        .forEach(scopeType => {
            const refs = flatten(schemaConfigs.map(c => c.references
                .filter(x => x.scope & scopeType)
                .map(x => `${constants.schemasBaseUri}/${c.relativePath}#/${x.reference}`)));
            if (refs.length > 0) {
                if (!refsMap.has(scopeType)) {
                    refsMap.set(scopeType, []);
                }
                refsMap.get(scopeType)?.push(...refs)
            }
        });
    refsMap.forEach(async (refs, scopeType) => {
        const rootSchemaConfig = RootSchemaConfigs.get(scopeType) as RootSchemaConfiguration;
