...
import os from 'os';
import { findDirRecursive, rmdirRecursive, lowerCaseCompare, lowerCaseCompareLists, lowerCaseStartsWith, readJsonFile, writeJsonFile, safeMkdir, safeUnlink, lowerCaseEquals, executeCmd, findRecursive } from './utils';
...
export async function generateSchemasWithCsharpGenerator(providerNamespace: string): Promise<SchemaConfiguration[]> {
    const tmpFolder = path.join(os.tmpdir(), Math.random().toString(36).substr(2));
    const namespace = providerNamespace;
    const bicepTypesGeneratedFolder = path.resolve(constants.repoRoot, 'bicep-types-az/generated');
    const csharpGeneratorExe = path.resolve(constants.repoRoot, 'src/TemplateSchemaGenerator/bin/Release/net10.0/publish/TemplateSchemaGenerator');
    const schemaConfigs: SchemaConfiguration[] = [];

    console.log(`Running C# generator for namespace: ${namespace}`);

    try {
        await mkdir(tmpFolder, { recursive: true });
        await executeCmd(constants.repoRoot, csharpGeneratorExe, [
            '--source-folder', bicepTypesGeneratedFolder,
            '--output-folder', tmpFolder,
            '--provider-namespace', namespace,
            '--no-update-root-schemas'
        ]);

        // Discover the schemas written by the C# generator for this namespace and build SchemaConfiguration entries.
        const namespaceLower = namespace.toLowerCase();
        const schemaFiles = await findRecursive(tmpFolder, p =>
            path.basename(p).toLowerCase() === `${namespaceLower}.json`);

        for (const schemaFilePath of schemaFiles) {
            const relativePath = path.relative(tmpFolder, schemaFilePath).replace(/\\/g, '/');
            const schema = await readJsonFile(schemaFilePath);

            const references: SchemaReference[] = [
                ...getSchemaRefs(schema, ScopeType.Tenant, 'tenant_resourceDefinitions'),
    ...
    console.log(`C# generator produced ${schemaConfigs.length} schema file(s) for ${namespace}`);
    return schemaConfigs;
}
...
async function getDisabledProviderRefs(autoGenList: AutoGenConfig[], scopeType: ScopeType, rootSchemaConfig: RootSchemaConfiguration) {
    const currentRefs = await getCurrentTemplateRefs(scopeType, rootSchemaConfig);
    const disabledFiles = new Set(autoGenList.filter(x => x.disabledForAutogen).map(x => getSchemaFileName(x.namespace, x.suffix).toLowerCase()));
    
    // Return refs that belong to disabled provider schema files
    return currentRefs.filter(ref => {
        const filePath = getFilePathFromRef(ref);
        const fileName = path.basename(filePath).toLowerCase();
        return disabledFiles.has(fileName);
    });
}

export async function clearAutoGeneratedSchemaRefs(autoGenList: { namespace: string, suffix?: string }[]) {
...
export async function saveAutoGeneratedSchemaRefs(schemaConfigs: SchemaConfiguration[], isGenerateAll: boolean, preserveDisabledProviders?: AutoGenConfig[]) {
    for (const [scopeType, rootSchemaConfig] of RootSchemaConfigs) {
        ...
        let currentRefs = !isGenerateAll ? await getCurrentTemplateRefs(scopeType, rootSchemaConfig): [];
        
        // In batch mode (isGenerateAll=true), preserve disabled provider refs
        if (isGenerateAll && preserveDisabledProviders) {
            const disabledRefs = await getDisabledProviderRefs(preserveDisabledProviders, scopeType, rootSchemaConfig);
            currentRefs = disabledRefs;
        }
