...
export const postProcessor: SchemaPostProcessor = async (namespace, apiVersion, schema) => {
  ...
  replaceCyclicRef(schema.definitions?.PublicIPAddressPropertiesFormat?.properties?.oneOf[0]?.properties?.linkedPublicIPAddress?.oneOf[0]);
  replaceCyclicRef(schema.definitions?.PublicIPAddressPropertiesFormat?.properties?.oneOf[0]?.properties?.servicePublicIPAddress?.oneOf[0]);
