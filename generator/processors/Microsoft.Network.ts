...
export const postProcessor: SchemaPostProcessor = async (namespace, apiVersion, schema) => {
  ...
  replaceCyclicRef(schema.definitions?.PublicIPAddressPropertiesFormat?.properties?.linkedPublicIPAddress?.oneOf[0]?.items);
