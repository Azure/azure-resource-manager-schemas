...
internal class TestSchemaCache : INormalizedSchemaCache
{
    ...
    /// <summary>
    /// Initializes the schema cache from a set of file paths. Parsing and normalization for a given provider
    /// is deferred until that provider is actually requested, so the full schema corpus is never fully
    /// parsed/normalized in memory at once.
    /// </summary>
    ...
    private TestSchemaCache(IEnumerable<string> filePaths)
    {
        ...
        // Only the small, fixed whitelist of top-level/common schemas is parsed up front. Their $ref lists are
        // what the external SchemaUtils normalization uses to discover which schema keys exist and how they
        // group together (e.g. Microsoft.Compute.json + Microsoft.Compute.Extensions.json), so we reuse the
        // same discovery logic here without ever parsing the full (multi-thousand-file) schema corpus.
    ...
    /// <summary>
    /// All provider keys discovered from the supplied file paths, excluding the shared common/top-level schemas.
    /// Used by tests that need to validate every provider without holding the whole corpus in memory at once.
    /// </summary>
