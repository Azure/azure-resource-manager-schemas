...
public static class JsonSchemaGenerator
{
    ...
    public static JsonNode GenerateProviderSchema(string providerNamespace, string apiVersion, IEnumerable<ResourceType> resourceTypes, bool sortProperties)
    {
    ...
    private sealed class JsonSchemaConverter
    {
        ...
        private static readonly HashSet<string> ReservedResourcePropertyNames = new(StringComparer.OrdinalIgnoreCase) { "type", "apiVersion" };
        ...
        private JsonObject ConvertDiscriminatedObjectTypeResourceBody(DiscriminatedObjectType discriminatedBody, string resourceTypeName, string apiVersion)
        {
            ...
            foreach (var (_, element) in discriminatedBody.Elements.OrderBy(x => x.Key, StringComparer.OrdinalIgnoreCase))
            {
                ...
                var filteredElementProperties = elementObjectType.Properties.Where(p => !ReservedResourcePropertyNames.Contains(p.Key));
                var (properties, required) = ConvertWritableProperties(filteredElementProperties, []);
        ...
        private static void RewriteNameEnumToFullNamePattern(JsonObject properties, string typeName)
        {
            // Only child resources (3+ segments) need the full-name prefix pattern.
            if (typeName.Split('/').Length < 3)
            {
                return;
            }

            if (properties["name"] is not JsonObject { } nameSchema ||
                nameSchema["oneOf"] is not JsonArray { Count: >= 1 } oneOfArray ||
                oneOfArray[0] is not JsonObject { } firstOption ||
                firstOption["enum"] is not JsonArray { Count: 1 } enumArray ||
                enumArray[0]?.ToString() is not { Length: > 0 } literal)
            {
                return;
            }
        ...
        private JsonObject AddArmExpressionToPropertySchema(string propertyName, TypeBase type, bool allowRef, HashSet<TypeBase> stack, bool treatTypeAndApiVersionAsStrict)
        {
            ...
            return new()
            {
                ["oneOf"] = new JsonArray(
                    schema,
                    new JsonObject { ["$ref"] = ArmExpressionRef }),
            };
        }
        ...
        private (JsonObject properties, JsonArray required) ConvertWritableProperties(IEnumerable<KeyValuePair<string, ObjectTypeProperty>> properties, HashSet<TypeBase> stack)
        {
            ...
            foreach (var (propName, prop) in properties.OrderBy(x => x.Key, StringComparer.OrdinalIgnoreCase))
            {
                ...
                var propSchema = AddArmExpressionToPropertySchema(propName, prop.Type.Type, allowRef: true, stack, treatTypeAndApiVersionAsStrict: false);
                if (!string.IsNullOrWhiteSpace(prop.Description))
                {
                    propSchema.Insert(0, "description", prop.Description);
                }
