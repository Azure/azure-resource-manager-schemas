    ...
    ResourceTypeProvider resourceTypeProvider)
{
    ...
    public record Options(string OutputFolder, string? ProviderNamespace, bool UpdateRootSchemas);

    private void WriteJsonFile(Options options, string path, JsonNode content)
    {
        var filePath = Path.Combine(options.OutputFolder, path);
        fileSystem.Directory.CreateDirectory(Path.GetDirectoryName(filePath)!);
        fileSystem.File.WriteAllText(filePath, content.ToJsonString(SerializerOptions));
    }
    ...
    public void Generate(Options options)
    {
        ...
        if (options.UpdateRootSchemas)
        {
            var autogeneratedResources = AutogeneratedResourcesGenerator.Generate([..autogeneratedRefs]);
            WriteJsonFile(options, "common/autogeneratedResources.json", autogeneratedResources);
        }
    }
