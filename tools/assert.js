...
function Same(lhs, rhs, message) {
  if (lhs != rhs) {
    if (!message) {
      message = "Expected lhs to reference the same value as rhs.";
    }
    Fail(ExpectedActualString(lhs, rhs, message));
  }
}

module.exports.NotSame = NotSame;
function NotSame(lhs, rhs, message) {
  if (lhs == rhs) {
    if (!message) {
      message = "Expected lhs to not reference the same value as rhs.";
    }
    Fail(ExpectedActualString(lhs, rhs, message));
  }
}
...
function NotEqual(lhs, rhs, message) {
  if (utilities.equals(lhs, rhs)) {
    Fail(ExpectedActualString(lhs, rhs, message));
  }
}
